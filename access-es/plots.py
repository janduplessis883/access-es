...
def create_weekly_trend_plot(
    weekly_agg, show_per_1000, list_size, arrs_end_date, should_apply_arrs
):
    ...
    if show_per_1000:
        ...
        fig = px.line(
            ...
            y=y_plot,
            title=plot_title,
            labels={"week": "Week Starting Date", y_plot: "Apps per 1000 per week"},
        ...
        fig = px.bar(
            ...
            title=plot_title,
            labels={"week": "Week Starting Date", "value": "Count", "variable": "Type"},
            height=WEEKLY_PLOT_HEIGHT,
            color_discrete_map={
        ...
        fig.update_traces(
            ...
            text=(weekly_agg_plot[y_plot] + weekly_agg_plot["arrs_historical"]).round(
                0
            ),
            textposition="inside",
        )
...
def create_monthly_trend_plot(monthly_agg, arrs_end_date, should_apply_arrs):
    ...
    fig.update_traces(
        selector=dict(name="arrs_historical"),
        text=(
            monthly_agg_plot["total_appointments"] + monthly_agg_plot["arrs_historical"]
        ).round(0),
        textposition="inside",
    )
    fig.update_traces(
...
def create_projection_chart(combined_proj_df, list_size):
    ...
    fig.update_traces(
        ...
        text=(
            combined_proj_df["total_appointments"] + combined_proj_df["Added (Exp)"]
        ).round(0),
        textposition="inside",
    )
    fig.update_traces(
        ...
        text=(
            combined_proj_df["total_appointments"]
            + combined_proj_df["Added (Exp)"]
            + combined_proj_df["ARRS"]
        ).round(0),
        textposition="inside",
    )
    fig.update_traces(
        ...
        text=(
            ...
            + combined_proj_df["Added (Exp)"]
            + combined_proj_df["ARRS"]
            + combined_proj_df["Catch-up Needed"]
        ).round(0),
        textposition="inside",
    )
...
def create_clinician_stats_histograms(stats_df):
    ...
    sns.histplot(
        data=stats_df, x="Total Apps", ax=axes[0], color="#1f77b4", kde=True, bins=15
    )
    ...
    # DNAs histogram
    sns.histplot(
        data=stats_df, x="DNAs", ax=axes[1], color="#d62728", kde=True, bins=15
    )
    ...
    axes[2].set_title(
        "Avg App Duration (mins) Distribution", fontsize=12, fontweight="bold"
    )
    ...
    axes[3].set_title(
        "Avg Book to App (mins) Distribution", fontsize=12, fontweight="bold"
    )
