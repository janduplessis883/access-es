...
def load_and_combine_csv_files(uploaded_files):
    ...
    for uploaded_file in uploaded_files:
        try:
            ...
            file_info.append(
                {"name": uploaded_file.name, "rows": len(df), "success": True}
            )
        except Exception as e:
            file_info.append(
                {"name": uploaded_file.name, "error": str(e), "success": False}
            )
    ...
    if "appointment_date" in df.columns:
        try:
            df["appointment_date"] = pd.to_datetime(
                df["appointment_date"], format="%d-%b-%y"
            )
        except ValueError:
            df["appointment_date"] = pd.to_datetime(
                df["appointment_date"], format="mixed", dayfirst=True
            )
...
def filter_dataframe(
    df, selected_clinicians, selected_rota_types, date_range=None, exclude_dna=True
):
    ...
    if exclude_dna:
        ...
        dna_percentage = (
            (dna_count / dna_count_before * 100) if dna_count_before > 0 else 0
        )
...
def calculate_arrs_values(
    arrs_2526, arrs_month, arrs_future, slider_end_date, arrs_end_date, weeks_elapsed
):
    ...
    slider_end_ts = (
        pd.Timestamp(slider_end_date)
        if not isinstance(slider_end_date, pd.Timestamp)
        else slider_end_date
    )
...
def create_weekly_aggregation(df, list_size, arrs_params, cutoff_date):
    ...
    weekly_agg["arrs_historical"] = np.where(
        weekly_agg["week"] < cutoff_date, arrs_params["estimated_weekly_arrs"], 0
    )
    ...
    weekly_agg["total_with_forecast"] = (
        weekly_agg["total_with_arrs"] + weekly_agg["forecasted_apps"]
    )
...
def create_monthly_aggregation(df, list_size, arrs_params, cutoff_date):
    ...
    monthly_agg = (
        monthly_df.groupby("month").size().reset_index(name="total_appointments")
    )
    ...
    if arrs_params["arrs_future"] and months_after_cutoff > 0:
        monthly_post_cutoff_increment = (
            arrs_params["future_arrs_apps"] / months_after_cutoff
        )
    else:
    ...
    monthly_agg["arrs_future"] = np.where(
        monthly_agg["month"] >= cutoff_date, monthly_post_cutoff_increment, 0
    )
    ...
    monthly_agg["total_with_arrs"] = (
        monthly_agg["total_appointments"] + monthly_agg["total_arrs_estimated"]
    )

    # Total including forecasts (when implemented)
    monthly_agg["total_with_forecast"] = (
        monthly_agg["total_with_arrs"] + monthly_agg["forecasted_apps"]
    )
...
def calculate_clinician_stats(df_with_dna, selected_clinicians):
    ...
    if clinician_stats:
        ...
        stats_df = stats_df.sort_values(by="Total Apps", ascending=False).reset_index(
            drop=True
        )
        return stats_df
...
def calculate_target_achievement(df, list_size, arrs_values, exp_add_apps_per_week=0):
    ...
    projected_future_arrs = (
        arrs_values["estimated_weekly_arrs"] * weeks_from_arrs_to_end
    )
    ...
    total_baseline_projection = (
        achieved_so_far + projected_surgery_baseline + projected_future_arrs
    )
...
def create_projection_dataframe(
    weekly_agg, target_metrics, arrs_values, exp_add_apps_per_week, forecast_df=None
):
    ...
    future_weeks_list = pd.date_range(
        start=last_historical_week + pd.Timedelta(weeks=1), end=FY_END, freq="W-MON"
    )
    ...
    if (
        ...
        and "week" in forecast_df.columns
    ...
    for w in future_weeks_list:
        remaining_gap = max(
            0, target_metrics["required_extra_per_week"] - exp_add_apps_per_week
        )
        ...
        if week_key in forecast_map:
            ...
            forecast_type = (
                "Projected Baseline" if not forecast_available else "Forecasted"
            )
    ...
    if future_data:
        ...
        if forecast_available:
            print(
                f"✅ Matched {matched_count} out of {len(future_weeks_list)} projection weeks with forecast data"
            )
