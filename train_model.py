    ...
    if not parsed_successfully:
        try:
            ...
            if valid_count > len(df) * 0.5:
                ...
                print(f"Successfully parsed {valid_count}/{len(df)} dates using auto-detection")
        except:
            pass
...
def prepare_training_data_robust(
    ...
    for file in training_files:
        try:
            ...
            if date_column not in df.columns or appointments_column not in df.columns:
                print(f"Required columns not found in {file.name}. Available: {df.columns.tolist()}")
                continue
            ...
            if removed_count > 0:
                print(f"Removed {removed_count} rows with invalid dates or missing appointments from {file.name}")
    ...
    print(f"Successfully prepared {len(weekly_df)} weeks of training data from {len(all_dfs)} files")
    print(f"Date range: {weekly_df['week'].min()} to {weekly_df['week'].max()}")
    
    return weekly_df


def prepare_influenza_data(
    ...
    try:
        ...
        print(f"Date range: {weekly_influenza['week'].min()} to {weekly_influenza['week'].max()}")
...
def prepare_training_data(
    ...
    if os.path.exists(training_data_path):
        ...
        if date_column in hist_df.columns and appointments_column in hist_df.columns:
            hist_df[date_column] = pd.to_datetime(hist_df[date_column], format='%d-%b-%y', errors='coerce')
            hist_df = hist_df.rename(columns={date_column: 'date', appointments_column: 'appointments'})
            # Filter out any rows with NaT dates
            hist_df = hist_df.dropna(subset=['date'])
            all_training_dfs.append(hist_df[['date', 'appointments']])
        else:
            print(f"Warning: Columns '{date_column}' or '{appointments_column}' not found in {training_data_path}")
    ...
    if filtered_df is not None and len(filtered_df) > 0:
        ...
        daily_current = current_df.groupby('date').size().reset_index(name='appointments')
    ...
    if additional_training_files:
        for file in additional_training_files:
            try:
                ...
                app_col = [c for c in add_df.columns if 'app' in c.lower() or 'used' in c.lower()][0]
                
                add_df[date_col] = pd.to_datetime(add_df[date_col])
                add_df = add_df.rename(columns={date_col: 'date', app_col: 'appointments'})
...
def train_nbeats_model(
    ...
    try:
        ...
        influenza_weekly = influenza_df.groupby('week')['influenza'].mean().reset_index()
        ...
        pipeline = Pipeline([
            BoxCox(lmbda=0),  # Log transformation
            Scaler(MinMaxScaler())
        ])
        ...
        model = NBEATSModel(
            input_chunk_length=min(input_chunk_length, len(target_transformed) - validation_length),
