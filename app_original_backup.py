...
if uploaded_files:
    ...
    if dataframes:
        ...
        if drop_duplicates:
            st.badge(f":material/done_outline:  Dropped **{combined_df.duplicated().sum()}** duplicate rows.", color='blue')
        ...
        if has_required_cols:
            ...
            if selected_clinicians and selected_rota_types:
                ...
                if exclude_did_not_attend:
                    ...
                    st.badge(f":material/done_outline: Excluded {dna_count_excluded} 'Did Not Attend' appointments ({dna_percentage:.1f}% of total)", color='green')
