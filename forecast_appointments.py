...
def forecast_surgery_appointments(
    ...
    try:
        ...
        if os.path.exists(training_data_path):
            ...
            hist_df = hist_df.rename(columns={'Date': 'date', 'used_apps': 'appointments'})
            # Aggregate to weekly
        ...
        if len(all_data_dfs) > 0:
            ...
            appointments_df = appointments_df.drop_duplicates(subset=['date'], keep='last')
        else:
            appointments_df = current_df
        
        # 2. Load influenza data
        if not os.path.exists(influenza_csv_path):
            print(f"Influenza data not found at {influenza_csv_path}. Using appointments only.")
        ...
        influenza_df = influenza_df[
            (influenza_df['date'] >= min_date) & 
            (influenza_df['date'] <= max_date)
        ]
        ...
        if len(combined_df) < input_chunk_length:
            print(f"Insufficient data for forecasting. Need at least {input_chunk_length} weeks.")
            return None
        ...
        forecast_df['forecasted_appointments'] = forecast_df['forecasted_appointments'].round(0)
